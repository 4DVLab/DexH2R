...
from diffusion_policy_3d.env_runner.rot6d import *
...
def dis_loss(dis_points, obj_pcd: torch.Tensor, thres_dis = 0.2):
    '''
    cd loss
...
class hand_loss:
    ...
    def cal_loss(self,
        ...
        #         # 检查并创建保存目录
        #         object_path = f"{save_dir}/{object_dir}"
        #         gt_hand_path = f"{save_dir}/{gt_hand_dir}"
        #         pred_hand_path = f"{save_dir}/{pred_hand_dir}"
        #         final_hand_path = f"{save_dir}/{final_hand_dir}"

        #         # 创建目录（如果不存在）
        #         os.makedirs(object_path, exist_ok=True)
        #         os.makedirs(gt_hand_path, exist_ok=True)
        #         os.makedirs(pred_hand_path, exist_ok=True)
        #         os.makedirs(final_hand_path, exist_ok=True)
        #         # save object
...
class Evaluator:
    ...
    # # linear interpolate between the chosen final grasp and the current agent_pos
    # final_grasp_pos: [1, 28]  cur_agent_pos: [1, 28]

    def intersect_vox(self, obj_points: torch.Tensor, hand_mesh, pitch: float = 0.005) -> torch.Tensor:
        # Check if points are inside the hand mesh
        inside = torch.tensor(hand_mesh.contains(obj_points.cpu().numpy()), device=obj_points.device, dtype=torch.float32)
        # Calculate the volume of intersection
        volume = inside.sum() * (pitch ** 3)
        return volume

    def create_folders(self,base_folder, sub_folders):
        """
        创建指定的目录及其子目录，并返回所有子目录的路径。
        Args:
            base_folder (str): 基础目录路径。
            sub_folders (list): 需要在基础目录下创建的子目录列表。
        Returns:
            list: 所有创建的子目录的完整路径列表。
        """
    ...
    def preprocess_batch(self, batch, device, velocity_as_obs, finalgrasp_as_obs):
        """
        ...
        processed_data = {
            ...
            "objpcd_intact": batch["objpcd_intact"].squeeze(0),  # [16, 4096, 3]
        ...
        if velocity_as_obs:
            ...
            processed_data["velocity"] = batch["velocity"].squeeze(0) # [1, 16, 28] -> [16, 28]
        if finalgrasp_as_obs:
            processed_data["final_grasp"] = obs_dict["final_grasp"].squeeze(0) # [1, 16, 28] -> [16, 28]
        else:
            processed_data["final_grasp"] = batch["final_grasp"].squeeze(0) # [1, 16, 28] -> [16, 28]

        return processed_data

    def update_obs_dict(self, obs_dict, n_obs_steps, agent_pos_list, velocity_list, final_grasp_list):
        """
        ...
        Parameters:
            ...
            n_obs_steps (int): Number of observation steps to update.
            agent_pos_list (list): List of tensors for agent positions.
            velocity_list (list): List of tensors for velocities.
            final_grasp_list (list): List of tensors for final grasp values.
        ...
        for key, value in obs_dict.items():
            ...
            if key == "agent_pos":
                new_obs_dict[key][:, :n_obs_steps, :] = torch.stack(agent_pos_list, dim=1)
            elif key == "velocity":
                new_obs_dict[key][:, :n_obs_steps, :] = torch.stack(velocity_list, dim=1)
            elif key == "final_grasp":
                new_obs_dict[key][:, :n_obs_steps, :] = torch.stack(final_grasp_list, dim=1)
        return new_obs_dict
    ...
    def find_nearest_final_qpose(self,qpose,target_qpose_pool):
        '''
        param:
            qpose:              [seq_len,30]
    ...
    def compute_pose_distance(self,pose1, pose2):
        ...
        #rot_dist = torch.acos(torch.clamp((R_diff.trace() - 1) / 2, -1, 1))
        # 可以为旋转和平移设置不同的权重
        w_trans, w_rot = 1.0, 1.0
        return w_trans * trans_dist

    def slerp_quat(self, v1_, v2_, inter_num):
        '''
        param:
    ...
    def interpolate_motion(self,v1, v2, _num_steps=20):
        '''
        param:
            v1,v2 need to
        ...
        interpolate_motion[...,3:6] = quaternion_to_axis_angle(self.slerp_quat(v1_quat,v2_quat,_num_steps))

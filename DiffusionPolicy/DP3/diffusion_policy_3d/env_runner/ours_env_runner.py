...
class FastGraspEnvRunner(BaseRunner):
    ...
    def run_ours(self, policy: BasePolicy, test_dataloader) -> Dict:
        ...
        # some hyperparameter settings: unit: (m)
        gt_total_pener_frame = 0  # the calculated total_pener_frame should subtract this item
        gt_max_pener_depth = 0  # the calculated max_pener_depth should subtract this item
        total_infer_frame_threshold = 45 # we will interpolate iff total_infer_frame > total_infer_frame_threshol
        interpolation_threshold = 1
        evaluator = Evaluator(device)
        metrics = evaluator._initialize_metrics()
        self.hand_model = get_e3m5_handmodel(1, device)
        # Lists to store metrics for all trajectories
        # 初始化存储所有轨迹指标的字典
        all_metrics_lists = {
            "total_infer_frame": [],
            "inference_once_time": [],
        ...
        traj_num = 0
        last_traj_index = 5 # used to determine whether the traj has changed to another
        ...
        qpose_dim = 28
        inter_begin_pose = torch.zeros((1, qpose_dim)).to(device)
        ...
        # visualized list: pred_hand和itp_hand放在不同的List里，因为之后dp infer的hand和itp_hand的颜色不一样
        # 每个traj infer结束后统一进行visualize
        visualizations = {
            "pred_action": [],
            "gt_action": [],
            "final_grasp": [],

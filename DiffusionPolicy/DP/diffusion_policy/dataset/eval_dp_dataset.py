...
class eval_dp_dataset(BaseImageDataset):
    def __init__(self,
            zarr_path, 
            traj_num,
            train_mask_start,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path, keys=['image','agent_pos','action','depth','objpcd_intact', 'objpcd_normal_intact', 'velocity','final_grasp','final_grasp_group','traj_index','obs_objpcd'])
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        ...
        normalizer['depth'] = get_image_range_normalizer()
        return normalizer    
    ...
    def _sample_to_data(self, sample):
        rgb_image = np.moveaxis(sample['image'], -1, 1)/255
        depth_image = np.moveaxis(sample['depth'], -1, 1)/255
        ...
        obs_objpcd = sample['obs_objpcd'].astype(np.float32)
        ...
        data = {
            ...
            'objpcd_intact': objpcd_intact.transpose(0,2,1),
            'objpcd_normal_intact': objpcd_normal_intact.transpose(0,2,1),
            'obs_objpcd': obs_objpcd.transpose(0,2,1),
...
# TODO: test whether can load the zarr dataset; need to change the zarr_path
def test():
    import os

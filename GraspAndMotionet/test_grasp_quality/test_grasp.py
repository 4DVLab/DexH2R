...
from test_grasp_quality.grasp_test_force_shadow_hand import IsaacGraspTestForce_shadowhand as IsaacGraspTestForce
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--stability_config', type=str,
                        default='test_grasp_quality/stability_config.yaml',
...
def stability_tester(args: argparse.Namespace) -> dict:
    ...
    grasps = torch.load(os.path.join(args.eval_dir, 'res_data.pt'))
    isaac_env = None
    results = {}
    across_all_cases = 0
    across_all_succ = 0
    ...
    for object_name in grasps['sample_qpos'].keys():
        ...
        # load object mesh
        object_mesh_path = os.path.join('./../dataset/object_model', f'{object_name}.obj')
...
def load_obj_pcd_nor_dict(dir_path):
    obj_pcd_nor_dict = {}
    for pcd_file_name in os.listdir(dir_path):
        obj_pcd_path = pjoin(dir_path, pcd_file_name)
        surface_pcd = o3d.io.read_point_cloud(obj_pcd_path)
        ...
        obj_pcd_nor_dict[pcd_file_name] = torch.cat([obj_surface_pcd, obj_surface_pcd_normals], dim=1)
    return obj_pcd_nor_dict

def collision_tester(args: argparse.Namespace, stability_results: dict) -> None:
    _BATCHSIZE = 25 #NOTE: adjust this batchsize to fit your GPU memory && need to be divided by generated grasps per object
    ...
    #realdex
    obj_pcds_nors_dict = load_obj_pcd_nor_dict("./../dataset/obj_surface_pcd_4096/")
    ...
    for object_name in grasps['sample_qpos'].keys():
        ...
        for i in range(qpos.shape[0] // _BATCHSIZE):
            ...
            depth_collision = compute_collision(obj_pcd_nor.to(args.device), hand_surface_points)
